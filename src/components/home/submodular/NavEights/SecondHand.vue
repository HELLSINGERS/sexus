      ...
      <ul
        ...
        <li v-for="item in list" class="list_content" @click="goMain(item.id)">
          ...
          <div class="item_info">
            ...
            <span class="item_red">{{item.total_price}}万元</span>
...
<script>
  ...
  import {usedLists, area, price, houseType, direction, builtArea, label, age, floor, decoration, dianti, type} from '../../../../api/config/config'
  export default{
    data () {
      return {
        list: [],
    ...
    methods: {
      goMain (id) {
        this.$router.push({path: '/secondHandMain'})
        sessionStorage.setItem('secondHandId', id)
      },
      suress () {
        this.displays()
      ...
      prices (item) {
        ...
        if (item === this.price[0]) {
          ...
          this.total_price = '0-30'
        } else if (item === this.price[2]) {
          this.total_price = '30-40'
        } else if (item === this.price[3]) {
          this.total_price = '40-50'
        } else if (item === this.price[4]) {
          this.total_price = '50-60'
        } else if (item === this.price[5]) {
          this.total_price = '60-80'
        } else if (item === this.price[6]) {
          this.total_price = '80-100'
        } else if (item === this.price[7]) {
          this.total_price = '100-120'
        } else if (item === this.price[8]) {
          this.total_price = '120-10000'
        }
        this.getData()
      },
      contains (arr, obj) {
